# Format GitHub Issue Description Command
...
```bash
# Issue詳細情報（本文とコメント）
gh issue view $ARGUMENTS --json number,title,body,author,state,labels,comments,url

# 関連するPR情報（もしあれば）
gh issue view $ARGUMENTS --json closedBy
```
