...
- AWS の認証情報を要するコマンド (`aws`, `terraform`, `cdk`, `boto3` 等) は Claude が実行せずユーザーに委ねる (rules/role-separation.md 参照)。提示するコマンドは `aws-vault exec <profile> -- <command>` 形式で書く (平文 credential の利用を避ける)
