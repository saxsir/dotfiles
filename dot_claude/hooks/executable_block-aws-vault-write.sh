...
# Why-not: 初段は生のコマンド文字列を見るため、token-splitting (aws\-vault, aws-"v"ault
# のような quoting) で分断された呼び出しは検出できない。本 hook の脅威モデルは Claude が
# 正常動作で誤って非 readonly profile を選ぶ事故の防止であり、難読化・injection は対象外。
...
allowed_aws_vault=$(echo "$CMD" | grep -oiE "(^|[^A-Za-z0-9_.-])aws-vault[[:space:]]+(exec|login|export)[[:space:]]+(--?[A-Za-z0-9-]+=[^[:space:]]+[[:space:]]+)*[\"']?[A-Za-z0-9_.-]+-readonly[\"']?([[:space:]]|\$)" | wc -l)
allowed_avt=$(echo "$CMD" | grep -oiE "(^|[^A-Za-z0-9_-])avt[[:space:]]+[\"']?[A-Za-z0-9_.-]+-readonly[\"']?([[:space:]]|\$)" | wc -l)
