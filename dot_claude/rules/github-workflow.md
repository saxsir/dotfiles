...
本文は `.github/` にテンプレがあればそれに従う。無ければ「何をしたか」を数行と「なぜ必要か」、それに Issue との紐付けを書く。`Closes https://github.com/<owner>/<repo>/issues/<n>` を書けば merge 時に自動クローズされる。閉じたくないときは `Relates to <URL>` にするか `gh pr edit <n> --add-issue <n>` で紐付けだけ張る。参照は短縮形 `#n` ではなくフル URL で書く ([[github-writing]] の「根拠を示す」)。
