...
リンクは完全な URL で書く。GitHub は短縮形 `owner/repo#番号` も自動リンクするが、URL のほうが読み手が飛び先をひと目で判別できる。ソースコードの行位置は短縮形では書けないので、そもそも URL (commit SHA 入りの permalink) しか選択肢がない。同一リポ内の Issue / PR を指すだけなら `#番号` のままでよい。
