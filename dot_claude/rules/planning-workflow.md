...
実装者の分岐は「完了条件を宣言できる粒度まで分解済みか」の 1 軸だけで見る ([[delegation]] と同じ基準)。分解済み (チケットか計画がある) なら `delegate-issue` (チケットが無い単発は Agent tool を直接起こす)、未分解で小さいならソロで `implement`。context が逼迫してセッションを跨ぐときは `handoff`。

## 終了 / 改善

skill 自体を育てるときの執筆規範は `writing-for-agents`。
