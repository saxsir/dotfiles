...
description: 「学びを残して」「ルール化して」と指示されたときに、今回のタスクから「最初に知っていれば遠回りしなかった」知見を抽出し、ast-grep ルール / skill / CLAUDE.md(rules) ルールのいずれかに固定する。プロンプトに頼らず再現可能な形に落とすことを優先する。
model: sonnet
---
...
- 試行錯誤の末に解にたどり着いたとき（初手で詰まった、誤った仮説を立てた、ドキュメント不足で時間を溶かした 等）に、ユーザーから依頼されたとき
